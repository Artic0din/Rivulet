...
| **E4-PR5C** | Correct + wire `PlaybackFallbackPolicy` to live behaviour | Med | E4-PR5B | Behaviour must be identical | 5C — **DONE 2026-06-02** (corrected the inverted player model: AVKit→HLS-once, RPlayer→no-auto-fallback; wired `PlaybackFallbackPolicy` as the AVPlayer-path fallback decision source in `shouldAttemptRivuletFallbackOnItemFailure` + both `startWithFallback` startup catches — identical to the old `planHasHLSFallback && !attempted` gate; live `routeFellBack` telemetry emitted on the one-shot. `DEBT-E4-PR3-FALLBACK-001` closed; `DEBT-E4-PR3-001` reduced (fallback wired; route-family still deferred). `PlaybackPolicyIntegrationTests` + updated `PlaybackRoutingPolicyTests`/`PlaybackInterruptionRecoveryPolicyTests`) |
| **E4-DEVICE-GATE** | Media corpus + physical Apple TV readiness pack (docs only) | Low | E4-PR5C | None (planning) | **DONE 2026-06-03** — `e4-device-corpus-readiness-pack.md`: full corpus matrix (video/HDR/container, audio, subtitle, content-type with before/after routes + fallback + HDR/audio/sub expectations), Ryan library-inventory checklist, physical-device requirements, 16-step per-sample procedure, and the explicit **E4-PR6 Go/No-Go criteria**. Gates `DEBT-E1-PR1-004` + the device-environment gate on `DEBT-E4-PR3-001`/`DEBT-E4-PR5-001`. This is the E4-PR6 entry gate |
| **E4-PR6** | **AVKit-first default flip (staged, flag-gated)** | **High** | E4-PR3/PR4/PR5 + **E4-DEVICE-GATE** | **Ratification + corpus + device** (criteria in `e4-device-corpus-readiness-pack.md` §5) | 6 (highest-risk; latest) |

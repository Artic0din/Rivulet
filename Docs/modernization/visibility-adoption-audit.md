...
| ~~ScheduleLabelPolicy~~ | **RETIRED → superseded (ADO-03)** | `e53cf9f` → removed in ADO-03 | n/a | — | resolved | Replaced by the future-proof **Content Status Label System** (`ContentStatusLabel`/`ContentStatusPolicy`/`ContentStatusPlacement`). No built-but-unused schedule tagger remains |
| **Content Status Label System** | **LIVE on Hero + Detail (ADO-04)** | ADO-03 → ADO-04 (+ follow-up) | Home hero (`HeroSlideContent`) + show/movie detail page (`MediaDetailView`), chip above title | Home hero; open any show/movie detail | `DEBT-E3-ADO03-001` (episode-card labels, weekly cadence, renewed case, on-device visual) | TMDb status decoded (`TMDBStatusDetail`) + bridged (`TMDBContentStatus`) → label (New Season/Returns/Premieres/Coming/All Episodes Available). Proxy passthrough confirmed; `includeGuids=1` enabled everywhere so items carry a tmdbId. Appears only when data-backed; hidden otherwise |

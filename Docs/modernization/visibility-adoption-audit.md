...
| **Content Status Label System** | **LIVE on Hero (ADO-04, data-contingent)** | ADO-03 → ADO-04 | Home hero (`HeroSlideContent` chip above title) | Home → focus/auto-rotate hero | `DEBT-E3-ADO03-001` (proxy passthrough, detail/episode adoption, weekly cadence) | TMDb status decoded (`TMDBStatusDetail`) + bridged (`TMDBContentStatus`) → hero label (New Season/Returns/Premieres/Coming/All Episodes Available). Appears only when data-backed; hidden otherwise. Live appearance pending on-device confirmation that the backend `tmdb/details` proxy passes the fields |

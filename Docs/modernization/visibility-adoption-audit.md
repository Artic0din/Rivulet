...
| **poster-to-landscape-on-focus** | **LIVE, geometry corrected (ADO-02C)** | `e077384` (ADO-02) → ADO-02C | Home Recently Added row | Home → Recently Added (focus a card) | `DEBT-E3-PR7-001` (broader rows) | `.posterExpandsToLandscape`: **poster-shaped footprint at rest (no black gutters)** → landscape composition drawn as an **overflow overlay** on focus; stable footprint = no row reflow; focused cell `zIndex`-raised; pending Ryan's on-device visual confirmation |

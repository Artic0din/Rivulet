...
| A11Y-008 / A11Y-009 | 2026-06-01 | E3-PR4 | Code/policy audit (device pending) | Detail hero metadata cascade is now deterministic (type → genres → year·duration) via tested `DetailMetadataCascade`; VoiceOver reads a stable order. Detail primary actions and seasons/episodes structure unchanged. Device VoiceOver/focus capture pending (`DEBT-E0-007`). Evidence: `E3-PR4-CASCADE-001`, `E3-PR4-A11Y-001`. |
| A11Y-009 | 2026-06-01 | ADO-01 | Code/policy audit (device pending) | Live episode card (production `EpisodeCard`) play control now exposes a combined VoiceOver summary via `EpisodeCardPresentation` (episode, title, runtime, watched/progress) + "Plays this episode" hint; two-button play/info focus and selection unchanged. Device VoiceOver capture pending (`DEBT-E0-007`). Evidence: `ADO-01-ADOPT-001`, `ADO-01-A11Y-001`. |
| A11Y-010 | 2026-06-01 | E3-PR5 | Code/policy audit (device pending) | Discover now presents a calm, accessible loading/empty surface (shared `ContentStateView`, motion-free, combined VoiceOver element) instead of a blank screen; watchlist write feedback copy unchanged (already calm). Device capture pending (`DEBT-E0-007`). Evidence: `E3-PR5-PRESENT-001`, `E3-PR5-A11Y-001`. |
| A11Y-009 | 2026-06-01 | ADO-05 | Code/policy audit (device pending) | Episode-card content-status chip (Season Finale / New Episode Today / New Episode) is folded into the play control's existing combined VoiceOver summary **only when visible** (order: episode → title → status → runtime → watched/progress); the visible chip is `accessibilityHidden` so it is not announced twice, and nothing is announced when no chip shows. Play/info focus split, spoiler blur, and context menu unchanged. Device VoiceOver capture pending (`DEBT-E0-007`). Evidence: `ADO-05-A11Y-001`, `ADO-05-ADOPT-001`. |

...
## E4-PR5B — Wire Playback Policies (No AVKit Flip) (2026-06-02)

Four policy seams wired into live code as the single decision source, behaviour
preserved, avKitFirst off. Debts reduced (not closed).

| Evidence ID | Area | Date | Owner | Evidence | Source | Status | Reviewer | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| E4-PR5B-AUDIT-001 | Integration/Audit | 2026-06-02 | Epic 4 owner | Audited live decision sites (UPVM/ContentRouter/RivuletPlayer/DirectPlayPipeline/MediaDetailView); identified duplicated player-selection, resume-prompt, background-pause, and route/fallback decisions; produced the integration plan (4 safe seams + documented deferrals) | `playback-routing-policy.md` §Live, `playback-resume-policy.md` §8, `playback-interruption-recovery-policy.md` §9 | Reviewed | Pending | No behaviour changed during audit |
| E4-PR5B-PLAYER-001 | Architecture | 2026-06-02 | Epic 4 owner | `PlaybackRoutingPolicy.player()` now LIVE in `UniversalPlayerViewModel.startPlayback` (`RoutingInput(videoRequiresTranscode:, useApplePlayer:, avKitFirst:false)`); replaces `!useApplePlayer && !mustUseAVPlayer` | `UniversalPlayerViewModel.swift` (startPlayback) | Gate Satisfying | Pending | avKitFirst off → identical |
| E4-PR5B-RESUME-001 | Architecture | 2026-06-02 | Epic 4 owner | `PlaybackResumePolicy.decide()` now LIVE for the resume/restart prompt in `MediaDetailView.presentPlay`; prompt iff `.prompt(offsetMs)`; offsetMs == legacy `Int(offsetSec*1000)` | `MediaDetailView.swift` (presentPlay) | Gate Satisfying | Pending | ms conversion lossless (integer-ms Plex origin) |
| E4-PR5B-INTERRUPT-001 | Architecture | 2026-06-02 | Epic 4 owner | `PlaybackInterruptionRecoveryPolicy.decide(.appBackgrounded)` now LIVE in `observeAppLifecycle`; pause iff `.pauseAwaitingUser`; 1:1 phase map (`.ready→.loading`) | `UniversalPlayerViewModel.swift` (observeAppLifecycle, recoveryPhase) | Gate Satisfying | Pending | Branches only on `.playing` → identical |
| E4-PR5B-TELEMETRY-001 | Security/Observability | 2026-06-02 | Epic 4 owner | First LIVE playback telemetry: `routeSelected` emitted once per start from `prepareStreamURL` via `PlaybackTelemetry.emit`; anonymised route name, SafeContext descriptors, reasoning token; URL/token scrubbed at sink | `UniversalPlayerViewModel.swift` (emitRouteSelectedTelemetry) + `PlaybackPolicyIntegrationTests` | Reviewed | Pending | No URL/token expressible |
| E4-PR5B-FALLBACK-FINDING-001 | Architecture/Defect | 2026-06-02 | Epic 4 owner | Integration revealed `PlaybackFallbackPolicy` models avKit→noFallback, but the live AVPlayer path falls back to HLS once. Policy NOT wired (would change behaviour); model correction tracked `DEBT-E4-PR3-FALLBACK-001` | code review of `startWithFallback`/`attemptRivuletHLSFallback` | Reviewed | Pending | Honest non-wiring; no behaviour change |
| E4-PR5B-PRESERVE-001 | Behaviour | 2026-06-02 | Epic 4 owner | No default route flip, no player-default change, no playback/subtitle/audio/chapter/post-play UX change, no project-setting change, Epic 1 watch-state untouched | code review + `git diff` | Reviewed | Pending | Integration slice, behaviour-identical |
| E4-PR5B-TEST-001 | Testing | 2026-06-02 | Epic 4 owner | `PlaybackPolicyIntegrationTests` (11): player-selection matrix == legacy rule, RPlayer-first/useApple/transcode preserved, `isInProgress` ⇔ `MediaItem.isInProgress` incl. 98% boundary, prompt parity vs legacy `presentPlay`, prompt offset ms, background-pause parity across all phases, route-name anonymised, routeSelected fields secret-free. Ran with PlaybackRouting/Resume/Interruption/Telemetry/Observability + PlexProviderBoundary + ContentRouterPlaybackPlan | `xcodebuild … test` → ** TEST SUCCEEDED ** | Gate Satisfying | Pending | Behaviour-preservation contracts |
| E4-PR5B-BUILD-001 | Build | 2026-06-02 | Epic 4 owner | tvOS simulator build exit 0; `git diff --check` clean | `xcodebuild build` → ** BUILD SUCCEEDED ** | Reviewed | Pending | Pre-existing DEBT-E0-005 warnings only |

## E4-PR5C — Correct + Wire Playback Fallback Policy (2026-06-02)

Corrected the inverted `PlaybackFallbackPolicy` player model and wired it as the
AVPlayer-path fallback decision source. Behaviour preserved.

| Evidence ID | Area | Date | Owner | Evidence | Source | Status | Reviewer | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| E4-PR5C-AUDIT-001 | Behaviour/Audit | 2026-06-02 | Epic 4 owner | Audited all fallback paths: AVPlayer path (`startWithFallback` direct/remux catch + `AVPlayerItem` failure observer) reloads as Plex HLS once, gated by `planHasHLSFallback` + `hasAttemptedRivuletHLSFallback` one-shot; RPlayer path (`startRivuletPlayback` catch, `errorPublisher`, `handlePipelineError`) → `.failed`, NO auto fallback; HLS primary → no fallback; user retry (`retryPlayback`) is distinct (resets one-shot + restarts) | `playback-routing-policy.md` §Fallback correction | Reviewed | Pending | CLAUDE.md "RPlayer→HLS auto fallback" note is stale vs current code |
| E4-PR5C-MODEL-001 | Architecture/Defect | 2026-06-02 | Epic 4 owner | Corrected `PlaybackFallbackPolicy.decide`: rivulet → `.noFallback` (always); avKit + non-HLS family + HLS available + not attempted → `.fallback(.hls)`, else `.stopWithError`. (Was inverted: rivulet→fallback, avKit→noFallback) | `Rivulet/Services/Plex/Playback/Pipeline/PlaybackRoutingPolicy.swift` | Gate Satisfying | Pending | One-shot, loop-free |
| E4-PR5C-WIRE-001 | Architecture | 2026-06-02 | Epic 4 owner | `PlaybackFallbackPolicy` now LIVE: `avPlayerFallbackDecision(plan:)` drives `shouldAttemptRivuletFallbackOnItemFailure` + both `startWithFallback` startup catches; concurrency/dedup mechanics retained at call sites | `UniversalPlayerViewModel.swift` | Gate Satisfying | Pending | Identical to old `planHasHLSFallback && !attempted` gate |
| E4-PR5C-TELEMETRY-001 | Security/Observability | 2026-06-02 | Epic 4 owner | `routeFellBack` emitted LIVE from `attemptRivuletHLSFallback` (anonymised from/to + typed failure category; URL/token-free); `DirectPlayFailureKind`→`FailureCategory` mapper added | `UniversalPlayerViewModel.swift` + `PlaybackPolicyIntegrationTests` | Reviewed | Pending | No URL/token expressible |
| E4-PR5C-PRESERVE-001 | Behaviour | 2026-06-02 | Epic 4 owner | AVKit HLS one-shot fallback preserved; RPlayer terminal-on-failure preserved; one-shot guard preserved; no new/removed fallback paths; no route-default change; no AVKit-first flip | code review + `git diff` + tests | Reviewed | Pending | Behaviour-identical |

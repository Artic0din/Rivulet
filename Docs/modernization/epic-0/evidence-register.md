...
| E0-OBS-002 | Observability | 2026-05-31 | Project Owner | Player error path sends raw `stream_url` to Sentry extras | `Rivulet/Views/Player/UniversalPlayerViewModel.swift` | Captured | Pending | High-priority redaction target. **Update 2026-06-01 (readiness pass `E4-READY-OBS-001`):** the raw `stream_url` extra is no longer present (error scope logs title/type/ratingKey/start_offset only); remediation OBSERVED but NOT formally closed — E4-PR1 to add regression tests + finish `print()` scan before closing |

...
| ADO-04-PROXY-001 | Risk/Backend | 2026-06-01 | Epic 3 owner | Live label appearance depends on the server-side `tmdb/details/{id}` proxy passing the status fields through — NOT verifiable from the client here. If it projects a subset, decode yields nils and the hero shows no chip (graceful). On-device check is the confirmation | `Rivulet/Services/TMDB/TMDBDiscoverService.swift` | Pending | Pending | `DEBT-E3-ADO03-001` |

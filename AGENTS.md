...
SwiftLint and swift-format are not required merge gates until project-specific configs and a baseline are approved. Do not add them to branch protection or CI as required checks without explicit verification.

## PR Review And CI Model

- **Claude Code is the implementation agent** for Epic 2 onward.
- **Codex is the primary advisory AI reviewer in GitHub.** Codex review uses the Codex GitHub integration, automatic Codex reviews where enabled, or a manual `@codex review` PR comment.
- **Codex is not a merge authority.** Deterministic GitHub Actions and Ryan/project-owner review are the required merge authorities.
- **Sentry Seer is optional advisory review** for crash, performance, and release-risk signals.
- **Greptile is optional secondary advisory review** and must not be required by branch protection.
- Do not create a fake Codex review workflow. A custom Codex GitHub Actions workflow is only allowed if Ryan explicitly approves it after confirming the Codex GitHub integration cannot meet the need.
- AI findings must be addressed or explicitly dismissed, but AI checks must remain advisory and non-required.

## Review Guidelines

Codex should review as a principal Swift/tvOS architecture reviewer for the Rivulet modernization program.

Focus on:
- Regression risk.
- Architecture drift.
- Epic 0 gate violations.
- Missed tests.
- Privacy/security issues.
- Token leakage.
- Sentry/logging hygiene.
- Overbroad refactors.
- Scope creep beyond the current PR slice.
- Missing evidence IDs.
- Missing debt updates.
- Missing known limitations.
- Missing dependency assumptions.
- Mismatch between implementation and PR objective.
- Changed behavior not covered by tests.
- Claude Code implementation mistakes.

Rules:
- Do not redesign the roadmap.
- Do not request broad redesigns outside the PR scope.
- Do not treat AI review as merge authority.
- Findings must be actionable, scoped, and tied to changed lines where possible.
- Pre-existing issues outside the diff should become follow-up debt unless the PR worsens them.
- Prefer high-signal P0/P1 findings over nitpicks.
- Apply extra scrutiny to auth, tokens, URLs, Sentry, `PrivacyInfo.xcprivacy`, Top Shelf, Plex networking, credential storage, focus/navigation, playback routing, and release evidence.
